...
1. The optional `OLSConfig.spec.agenticOLS.terminalTTL` is a positive whole number of days (minimum 1). It has **no CRD default**. When absent, including when `spec.agenticOLS` is absent, the classic operator omits the `terminal-ttl-days` key from `lightspeed-agentic-configuration`.
...
3. `AgenticRun.spec.terminalTTL` is an optional **positive whole-day** request (minimum 1) for earlier cleanup. It replaces the old seconds-based `spec.ttlAfterTerminal` field, which is removed; the operator never fills the new field from cluster configuration. A run may request a shorter TTL; a request above the cluster ceiling is accepted but does not lengthen retention. Zero is invalid and no longer disables cleanup.
...
| Owner | Contract |
| --- | --- |
| `lightspeed-operator` | Validates `OLSConfig.spec.agenticOLS.terminalTTL`; publishes or removes `terminal-ttl-days` on the existing `lightspeed-agentic-configuration` ConfigMap. Does not compute run deadlines. |
